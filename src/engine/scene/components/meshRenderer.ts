...
class MeshRenderer extends Component {
  ...
  render(scene, camera, mainLight, addlights, projectors) {
    ...
    for (let sysu of systemUniforms) {
      switch (sysu) {
        ...
        case SystemUniforms.Object2World: {
          ...
          uniformContext[SystemUniforms.Object2World] =
            this._objectToWorld.elements;
          break;
        }
        case SystemUniforms.World2Object: {
          this._worldToObject.setInverseOf(this.node.worldMatrix); //TODO: 此矩阵缓存到node
          uniformContext[SystemUniforms.World2Object] =
            this._worldToObject.elements;
          break;
        }
    ...
    if (
      camera.target != null &&
      camera.target.texture2D == this.material.mainTexture
    ) {
      return;
    }
    ...
    for (let pass of this.material.renderPasses) {
      if (pass.lightMode == LightMode.ForwardBase && mainLight != null) {
        ...
        uniformContext[SystemUniforms.WorldLightPos] = [
          lightForward.x,
          lightForward.y,
          lightForward.z,
          0.0,
    ...
    if (projectors != null && projectors.length > 0) {
      ...
      for (let projector of projectors) {
        ...
        materialProj.renderPass(
          this.mesh,
          uniformContext,
          materialProj.renderPasses[0]
        );
      }
