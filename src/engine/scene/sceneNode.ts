...
type ComponentMap = Record<SystemComponents, Components>;
...
class SceneNode {
    ...
    addPointLight(color: number[], range: number) {
    ...
    lookAt(target: Vector3, up: Vector3, smoothFactor: number) {
        ...
        if (
            Math.abs(worldPos.x - target.x) < math.ZeroEpsilon &&
        ...
        if (this.getComponent(SystemComponents.Camera) || this.getComponent(SystemComponents.Projector)) {
    ...
    addComponent(type: SystemComponents, component: Components) {
    ...
    render(scene: Scene, camera: Camera, mainLight: Light, lights: Light[], projectors: Projector) {
        let renderer = this.components[SystemComponents.MeshRenderer];
        if (renderer && renderer instanceof MeshRenderer) {
            renderer.render(scene, camera, mainLight, lights, projectors);
        }
    }
}
