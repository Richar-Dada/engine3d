...
class SceneNode {
  ...
  lookAt(target, up, smoothFactor) {
    ...
    if (
      this.getComponent(SystemComponents.Camera) ||
    ...
    if (smoothFactor != null) {
      this.worldRotation = Quaternion.slerp(
        this.worldRotation,
        _tempQuat.clone(),
        smoothFactor
      );
