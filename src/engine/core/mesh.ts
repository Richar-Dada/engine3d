...
import { Shader, VertexFormat } from "..";

class Mesh {
    _vertexBuffer: VertexBuffer;
    _indexBuffer: IndexBuffer | null;
    _wireframe: boolean;
    constructor(vertexFormat: VertexFormat, wireframe = false) {
        this._vertexBuffer = new VertexBuffer(vertexFormat);
        this._indexBuffer = null;
        this._wireframe = wireframe;
    }

    setVertexData(semantic: string, data: Float32Array) {
        this._vertexBuffer.setData(semantic, data);
    }

    setTriangles(data: Uint16Array | Uint8Array) {
