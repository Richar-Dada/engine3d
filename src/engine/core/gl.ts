...
function init(canvasId: string, app, config = null) {
  if (canvasId != null) {
...
function glCheck() {
  ...
  glAbility.MAX_CUBE_MAP_TEXTURE_SIZE = gl.getParameter(
  ...
  glAbility.MAX_VERTEX_UNIFORM_VECTORS = gl.getParameter(
    gl.MAX_VERTEX_UNIFORM_VECTORS
  );
  glAbility.MAX_VARYING_VECTORS = gl.getParameter(gl.MAX_VARYING_VECTORS);
  glAbility.MAX_COMBINED_TEXTURE_IMAGE_UNITS = gl.getParameter(
    gl.MAX_COMBINED_TEXTURE_IMAGE_UNITS
  );
  glAbility.MAX_VERTEX_TEXTURE_IMAGE_UNITS = gl.getParameter(
    gl.MAX_VERTEX_TEXTURE_IMAGE_UNITS
  );
  glAbility.MAX_TEXTURE_IMAGE_UNITS = gl.getParameter(
    gl.MAX_TEXTURE_IMAGE_UNITS
  );
  glAbility.MAX_FRAGMENT_UNIFORM_VECTORS = gl.getParameter(
  ...
  if (glAbility.WebGL2) {
    ...
    glAbility.MAX_FRAGMENT_UNIFORM_COMPONENTS = gl.getParameter(
      gl.MAX_FRAGMENT_UNIFORM_COMPONENTS
    );
    glAbility.MAX_VERTEX_UNIFORM_COMPONENTS = gl.getParameter(
      gl.MAX_VERTEX_UNIFORM_COMPONENTS
    );
    glAbility.MAX_ARRAY_TEXTURE_LAYERS = gl.getParameter(
      gl.MAX_ARRAY_TEXTURE_LAYERS
    );
    glAbility.MIN_PROGRAM_TEXEL_OFFSET = gl.getParameter(
      gl.MIN_PROGRAM_TEXEL_OFFSET
    );
    glAbility.MAX_PROGRAM_TEXEL_OFFSET = gl.getParameter(
      gl.MAX_PROGRAM_TEXEL_OFFSET
    );
    glAbility.MAX_VARYING_COMPONENTS = gl.getParameter(
      gl.MAX_VARYING_COMPONENTS
    );
    glAbility.MAX_VERTEX_OUTPUT_COMPONENTS = gl.getParameter(
      gl.MAX_VERTEX_OUTPUT_COMPONENTS
    );
    glAbility.MAX_FRAGMENT_INPUT_COMPONENTS = gl.getParameter(
      gl.MAX_FRAGMENT_INPUT_COMPONENTS
    );
    glAbility.MAX_SERVER_WAIT_TIMEOUT = gl.getParameter(
    ...
    //Transform feedback
    glAbility.MAX_TRANSFORM_FEEDBACK_SEPARATE_COMPONENTS = gl.getParameter(
      gl.MAX_TRANSFORM_FEEDBACK_SEPARATE_COMPONENTS
    );
    glAbility.MAX_TRANSFORM_FEEDBACK_INTERLEAVED_COMPONENTS = gl.getParameter(
      gl.MAX_TRANSFORM_FEEDBACK_INTERLEAVED_COMPONENTS
    );
    glAbility.MAX_TRANSFORM_FEEDBACK_SEPARATE_ATTRIBS = gl.getParameter(
      gl.MAX_TRANSFORM_FEEDBACK_SEPARATE_ATTRIBS
    );

    //Uniforms
    glAbility.MAX_VERTEX_UNIFORM_BLOCKS = gl.getParameter(
      gl.MAX_VERTEX_UNIFORM_BLOCKS
    );
    glAbility.MAX_FRAGMENT_UNIFORM_BLOCKS = gl.getParameter(
      gl.MAX_FRAGMENT_UNIFORM_BLOCKS
    );
    glAbility.MAX_COMBINED_UNIFORM_BLOCKS = gl.getParameter(
      gl.MAX_COMBINED_UNIFORM_BLOCKS
    );
    glAbility.MAX_UNIFORM_BUFFER_BINDINGS = gl.getParameter(
      gl.MAX_UNIFORM_BUFFER_BINDINGS
    );
    glAbility.MAX_UNIFORM_BLOCK_SIZE = gl.getParameter(
      gl.MAX_UNIFORM_BLOCK_SIZE
    );
    glAbility.MAX_COMBINED_VERTEX_UNIFORM_COMPONENTS = gl.getParameter(
      gl.MAX_COMBINED_VERTEX_UNIFORM_COMPONENTS
    );
    glAbility.MAX_COMBINED_FRAGMENT_UNIFORM_COMPONENTS = gl.getParameter(
